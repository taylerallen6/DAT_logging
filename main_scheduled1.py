...
def check_repeat_loads(df):
  df = df[df.duplicated(subset=['origin', 'destination'])]
  df = df.reset_index()
  print(len(df))
  print(df[['load_id', 'origin', 'destination', 'company_name', 'earliest_availability']])


def check_loads(lookup_origin, all_loads_file):
  loads = dat_api1.get_loads(lookup_origin)
  print(len(loads))
  ...
  all_loads_df = pd.read_excel(all_loads_file)
  df = pd.concat([all_loads_df, df], ignore_index=True, sort=False)
  ...
  check_repeat_loads(df)


if __name__ == "__main__":

  all_loads_file = "Excel_files/all_loads.xlsx"

  if not os.path.isfile(all_loads_file):
    print("no file")
    with open(all_loads_file, "w") as f:
      pass
  ...
  for i in range(2):
    check_loads(lookup_origin, all_loads_file)